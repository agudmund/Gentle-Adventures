...
- **Reads its story from a ledger.** The quest's source of truth is a Google Sheet (the `Quest_Log` tab), reached through a tiny Apps Script proxy with raw `urllib`. The bundled quest in `data/quest.py` is the offline fallback. The story is editable state: it can be rewritten while the game lives on.
...
data/quest.py              The quest itself: scenes, choices, verify rules,
